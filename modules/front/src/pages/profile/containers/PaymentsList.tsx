...
const MethodBadge = ({ method }: { method: PaymentMethod }) => {
  ...
  const config = {
    stripe: {
      label: t("paymentsSection.methods.stripe"),
      icon: <StripeIcon />,
      className: "border-[#635bff]/20 bg-[#635bff]/10 text-[#635bff]",
    },
    paypal: {
      label: t("paymentsSection.methods.paypal"),
      icon: <PayPalIcon />,
      className: "border-[#003087]/20 bg-[#003087]/10 text-[#003087]",
    },
  };
...
export default function PaymentsList(): JSX.Element {
  ...
  const columns: ColumnDef<Payment, any>[] = [
    {
      header: t("paymentsSection.table.order"),
      accessorKey: "orderId",
      cell: ({ row }) => (
        <div>
    ...
    {
      header: t("paymentsSection.table.buyer"),
    ...
    {
      header: t("paymentsSection.table.amount"),
      accessorKey: "amount",
      cell: ({ row }) => (
    ...
    {
      header: t("paymentsSection.table.method"),
      accessorKey: "method",
      cell: ({ row }) => <MethodBadge method={row.original.method} />,
    },
    {
      header: t("paymentsSection.table.date"),
      accessorKey: "createdAt",
      cell: ({ getValue }) => {
        const value = getValue<string>();
        return (
          <span className="text-xs text-muted-foreground">
            {t("paymentsSection.timeAgo", { value: timeAgo(value) })}
          </span>
        );
      },
    },
    {
      header: t("paymentsSection.table.status"),
      accessorKey: "status",
  ...
  return (
    <section>
      <Heading
        level={2}
        title={t("paymentsSection.title")}
        description={t("paymentsSection.description")}
        underline
      />
      ...
      <DataTable
        ...
        searchPlaceholder={t("paymentsSection.searchPlaceholder")}
        ...
        actionsRow={({ row }) => {
          ...
          return [
            {
              tooltip: t("paymentsSection.actions.view"),
            ...
            ...(canMarkPaid
              ? [
                  {
                    tooltip: t("paymentsSection.actions.markPaid"),
            ...
            ...(canRefund
              ? [
                  {
                    tooltip: t("paymentsSection.actions.refund"),
                    leftIcon: <Wallet2Icon className="h-4 w-4" />,
              ...
              : []),
            {
              tooltip: t("paymentsSection.actions.contactBuyer"),
              leftIcon: <Phone className="h-4 w-4" />,
              colorScheme: "danger" as const,
              onClick: () => alert(`Contacter l'acheteur ${row.original.id}`),
            },
            {
              tooltip: t("paymentsSection.actions.downloadReceipt"),
